...
if (isset($_GET['order']) and $_GET['order'][0]['column'] != '') {
    ...
    //echo $sql_order;
} else $sql_order = ' ORDER `data_registrazione` DESC ';
...
//echo "SELECT anagrafica_agenti.*, t.nome AS nomeprov FROM {$table} {$joins} {$sql_where} {$sql_order} {$sql_limit}";
...
foreach ($recordquerymain as $row) {
    ...
    $db->query("SELECT COUNT(id) AS totale FROM anagrafica_scuole WHERE id_agente = '".$row['id']."' ");
    $temp_scuole=$db->single();
    $num_scuole=$temp_scuole['totale'];
    
    $db->query("SELECT SUM((anagrafica_corsi.costo/studente_corso.num_rate)*studente_corso.num_rate_pagate) AS totale 
                FROM studente_corso,anagrafica_scuole,anagrafica_corsi
                WHERE  anagrafica_corsi.id=studente_corso.id_corso
                AND studente_corso.id_scuola=anagrafica_scuole.id
                AND anagrafica_scuole.id_agente = '".$row['id']."' 
                 ");
    ...
    if($_SESSION['livello'] == "Admin"){ 
     $newrow[1] = "<a href='".BASE_URL."index_agente.php?val=".$row['id']."' target='_blank'>".$row['nome']."</a>";    
    }else{
     $newrow[1] = $row['nome'];
     }
    $newrow[2] = $row['email'];
    ...
    $newrow[4] = $row['piva'];
    $newrow[5] = $row['nomeprov'];
    $newrow[6] = $doc_id; 
    $newrow[7] = $tess_san;
    if($_SESSION['livello'] == "Admin"){ 
      $newrow[8] ='<a href="javascript:void(null)" class="deactivaten" data-stato="'.$row['attivo'].'" for="' . $row['id'] . '">' . $active . '</a>';
    }else{
      $newrow[8] ='';  
    }
    $newrow[9]=$num_scuole;
