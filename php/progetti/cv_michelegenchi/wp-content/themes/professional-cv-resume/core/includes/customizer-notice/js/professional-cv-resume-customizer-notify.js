...
(function (api) {
	...
	api.sectionConstructor['professional-cv-resume-customizer-notify-section'] = api.Section.extend(
		{
				...
				return true;
					...
					jQuery( document ).ready(
						function () {
							...
							jQuery( '.professional-cv-resume-customizer-notify-dismiss-recommended-action' ).click(
								function () {
									...
									jQuery.ajax(
										{
											type: 'GET',
											data: {action: 'professional_cv_resume_customizer_notify_dismiss_action', id: id, todo: action},
											dataType: 'html',
											url: professionalcvresumeCustomizercompanionObject.ajaxurl,
											beforeSend: function () {
												jQuery( '#' + id ).parent().append( '<div id="temp_load" style="text-align:center"><img src="' + professionalcvresumeCustomizercompanionObject.base_path + '/images/spinner-2x.gif" /></div>' );
											},
											success: function (data) {
												...
												var actions_count      = recommended_sction.find( '.professional-cv-resume-customizer-plugin-notify-actions-count' );
												var section_title      = recommended_sction.find( '.section-title' );
												jQuery( '.professional-cv-resume-customizer-plugin-notify-actions-count .current-index' ).text( index );
												container.slideToggle().remove();
												if (jQuery( '.professional-cv-resume-theme-recomended-actions_container > .epsilon-recommended-actions' ).length === 0) {

													actions_count.remove();

													if (jQuery( '.professional-cv-resume-theme-recomended-actions_container > .epsilon-recommended-plugins' ).length === 0) {
										...
										jQuery( '.professional-cv-resume-customizer-notify-dismiss-button-recommended-plugin' ).click(
											function () {
												var id = jQuery( this ).attr( 'id' ),
												action = jQuery( this ).attr( 'data-action' );
												jQuery.ajax(
										...
										function activatePlugin(url) {
										    ...
										    jQuery.ajax({
										        beforeSend: function() {
										            jQuery('.activate-now').replaceWith('<a class="button updating-message">' + professionalcvresumeCustomizercompanionObject.activating_string + '...</a>');
										        },
										        async: true,
										        type: 'GET',
										        url: url,
 ...
 *
 * @package professional_cv_resume
 */
...
jQuery( document ).ready(
	function ($) {
		...
		$( 'body' ).on(
			'click', ' .professional-cv-resume-install-plugin ', function () {
